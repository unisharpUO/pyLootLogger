...
class Armor:
    def __init__(self, _id):
        ...
        self.TotalStats = self.Strength + self.Intelligence + self.Dexterity + \
                          self.ManaBonus + self.HitPointBonus + \
                          self.StaminaBonus
    ...
    def Encoder(self):
        return {
            ...
            "Luck": str(self.Luck),
            "TotalStats": str(self.TotalStats)
        }
    ...
    def __str__(self):
        ...
        return ''.join('{}: {}\n'.format(key, val) for key, val in attribs)
